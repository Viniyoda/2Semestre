class Stack:
    ...
    É uma estrutura de dados linear de acesso restrito, na qual
    tanto a operação de inserção (tradicionalmente chamada de
    "push") quanto a operação de remoção ("pop") acontecem no
    final (ou topo). Em consequência, o funcionamento da pilha
    obedece ao princípio LIFO (Last In, First Out): o último
    ...
    def peek(self):
        """
        Método que permite consultar o valor que está no topo da pilha sem
        removê-lo
